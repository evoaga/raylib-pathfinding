...
auto RenderUISystem(entt::registry &registry) -> void
{
    auto drawSelectedObstacleInfo = [&](int entityID, const Vector3 &position, float size)
    {
        ...
        DrawText(TextFormat("Position: [%.2f, %.2f, %.2f]", position.x, position.y, position.z), 10, 50, 20, BLACK);
    ...
    auto renderHealthBars = [&](TransformComponent &transform, Health &health, CameraComponent &cameraComponent)
    {
    ...
    selectedObstacleView.each([&](auto entity, auto &transform, auto &, auto &selected)
                              {
        if (selected.isSelected)
        {
            drawSelectedObstacleInfo(static_cast<uint32_t>(entity), transform.position, transform.scale.x);
        } });
