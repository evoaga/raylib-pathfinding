...
auto UpdateMinionPath(entt::registry &registry, const Vector3 &playerPosition) -> void
{
    ...
    minionView.each(
        [&](entt::entity, auto &minion, auto &transform, auto &pathComponent)
        {
            (void)minion;
                ...
                if (isPathBlocked) {
                    ...
                    for (auto obstacleEntity : obstacleView) {
                        const auto &obstacleTransform =
                            obstacleView.get<TransformComponent>(obstacleEntity);
                    ...
                    Point const goal = {
                        pathComponent.goalPos.x, pathComponent.goalPos.y, pathComponent.goalPos.z};

                    pathComponent.path = thetaStar(
                        navMeshView.get<NavMeshComponent>(*navMeshView.begin()).mesh,
                        start,
                        goal,
                        navMeshView.get<NavMeshComponent>(*navMeshView.begin()).obstaclePolygons);
...
auto MoveMinion(entt::registry &registry) -> void
{
    ...
    minionView.each(
        [&](entt::entity, auto &minion, auto &transform, auto &speed, auto &pathComponent)
        {
            (void)minion;
            ...
            if (pathComponent.path.empty()) {
                const Vector3 direction =
                    Vector3Normalize(Vector3Subtract(pathComponent.goalPos, transform.position));
                ...
                if (distance < 0.1F) {
                    ...
                    const Vector3 movement =
                        Vector3Scale(normalizedDirection, speed.value * GetFrameTime());
                    transform.position = Vector3Add(transform.position, movement);
                }
            }
        });
