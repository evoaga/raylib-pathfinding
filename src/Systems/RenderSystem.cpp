...
auto RenderSystem(entt::registry &registry) -> void
{
    ...
    auto renderPlayers = [&](TransformComponent &transform)
    {
        ...
        DrawCubeWires(
            transform.position, transform.scale.x, transform.scale.y, transform.scale.z, DARKBLUE);
    };
    ...
    auto renderMinions = [&](TransformComponent &transform, Minion &minion)
    {
        ...
        DrawCubeWires(
            transform.position, transform.scale.x, transform.scale.z, transform.scale.y, MAROON);
    };

    auto renderObstacles =
        [&](TransformComponent &transform, Obstacle &obstacle, Selected &selected)
    {
        ...
        DrawCube(
            transform.position, transform.scale.x, transform.scale.y, transform.scale.z, color);
        DrawCubeWires(
            transform.position, transform.scale.x, transform.scale.y, transform.scale.z, wireColor);
        ...
        if (pathComponent.goalSet) {
            ...
            if (!pathComponent.path.empty()) {
                for (size_t i = 0; i < pathComponent.path.size() - 1; ++i) {
                    Vector3 const start = {
                        pathComponent.path[i].x, pathComponent.path[i].y, pathComponent.path[i].z};
                    Vector3 const end = {pathComponent.path[i + 1].x,
                                         pathComponent.path[i + 1].y,
                                         pathComponent.path[i + 1].z};
