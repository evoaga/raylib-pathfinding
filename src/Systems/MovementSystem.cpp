...
auto PathfindingSystem(entt::registry &registry) -> void
{
    ...
    playerView.each(
        [&](entt::entity, auto &transform, auto &pathComponent)
        {
            ...
            if (isPointInAnyPolygon(pathComponent.goalPos, navMeshComponent.obstaclePolygons)) {
                pathComponent.goalPos = findNearestValidPoint(
                    pathComponent.goalPos, navMeshComponent.obstaclePolygons, 0.5F);
            }
            ...
            if (isPathBlocked && pathComponent.updateRequested) {
                ...
                pathComponent.path = thetaStar(
                    navMeshComponent.mesh, start, goal, navMeshComponent.obstaclePolygons);
...
auto MovementSystem(entt::registry &registry) -> void
{
    ...
    playerView.each(
        [&](entt::entity, auto &transform, auto &speed, auto &pathComponent)
        {
            if (!pathComponent.goalSet) {
                return;
            }
            ...
            if (pathComponent.isPathBlocked && !pathComponent.path.empty()) {
                moveEntity({pathComponent.path[pathComponent.currentPathIndex].x,
