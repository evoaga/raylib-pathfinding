...
void initializeNavMesh(NavMesh& mesh, const std::vector<Point>& initialPoints, const std::vector<Vector3>& obstaclePositions, float obstacleSize, std::vector<Polygon>& polygons) {
    ...
    for (size_t i = 0; i < trianglesSize; i += 3) {
        ...
        for (const auto& obstacle : obstaclePositions) {
            if (!intersect1 && edgeIntersectsObstacle(e1, obstacle, obstacleSize)) intersect1 = true;
            if (!intersect2 && edgeIntersectsObstacle(e2, obstacle, obstacleSize)) intersect2 = true;
            if (!intersect3 && edgeIntersectsObstacle(e3, obstacle, obstacleSize)) intersect3 = true;
            if (intersect1 && intersect2 && intersect3) break;
        }

        if (!intersect1) mesh.addEdge(e1.p1, e1.p2, e1.cost);
        if (!intersect2) mesh.addEdge(e2.p1, e2.p2, e2.cost);
        if (!intersect3) mesh.addEdge(e3.p1, e3.p2, e3.cost);
